...
describe("Login Test", () => {
  ...
  it("should log in with valid credentials and show dasboard", () => {
    cy.intercept("POST", `${apiBaseURL}/login`, this.data.loginInfo).as(
      "postLoginData"
    );
    cy.intercept("GET", `${apiBaseURL}/dashboard`, this.data.dashBoardInfo).as(
      "getDashData"
    );
    ...
    cy.get(".pieHolder").should(
      "have.length",
      this.data.dashBoardInfo.body.pieDataArr.length
    );
    cy.wait(2000);
    cy.get("#logout").click();
  ...
  it("should get error message when entering invalid credentials", () => {
    cy.intercept("POST", `${apiBaseURL}/login`, this.data.badCreds).as("postLoginData");
    ...
    cy.get("#error .err-item")
      .first()
      .should("have.text", this.data.badCreds.body.error);
  });
});
